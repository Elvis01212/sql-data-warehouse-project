...
BEGIN
    ...
    BEGIN TRY
        ...
        cst_key,
        CASE 
	        WHEN cst_firstname != TRIM(cst_firstname) THEN TRIM(cst_firstname) --Removing Unwanted spaces
	        ELSE cst_firstname
        END AS cst_firstname,
        CASE 
	        WHEN cst_lastname != TRIM(cst_lastname) THEN TRIM(cst_lastname) --Removing Unwanted spaces
	        ELSE cst_lastname
        END AS cst_lastname,
        CASE
	        ...
	        ELSE 'n/a'
        END AS cst_marital_status,-- Normalizing marital status to a readable format
        CASE
        ...
        FROM(
	        SELECT
	        *,
	        ROW_NUMBER() OVER(PARTITION BY cst_id ORDER BY cst_create_date) flag_list
	        FROM bronze.crm_cust_info
	        WHERE cst_id IS NOT NULL 
	        )T
	        WHERE flag_list = 1 -- Removing id duplicates
